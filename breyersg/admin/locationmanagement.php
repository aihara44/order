<?php
    ...
    $qry = mysqli_query($con, "DELETE FROM lokasi WHERE lokasi_id ='" .mres($con, $_GET["delete_id"])."'");
                    ...
                    <tbody>
                       <?php
                        ...
                        if(isset($_POST["btn_search"])) {
                            $qry = mysqli_query($con, "SELECT * FROM lokasi WHERE lokasi LIKE '%".mres($con,$_POST["search_text"])."%' ORDER BY lokasi_id asc");
                        }else{
                            $qry = mysqli_query($con, "SELECT * FROM lokasi ORDER BY lokasi_id asc");
                        }
                        while($row=mysqli_fetch_array($qry,MYSQLI_ASSOC)){
                            echo '<tr>';
                            echo '<td>'.$row["lokasi"]."</td><td>".$row["tarikh"]."</td><td><a href='addlocation.php?edit_id=".$row["lokasi_id"]."' class='btn btn-primary'>Edit</a> | <a href='?delete_id=".$row["lokasi_id"]."' onclick=\"return confirm('Are you sure you want to delete this item?');\" class='btn btn-danger'>Delete</a></td>";
                            echo '</tr>';
                        }
                        ?>
