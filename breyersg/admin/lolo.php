<?php
    ...
    $depmq = mres($con, $_POST["depmq"]);
    
    $qry = mysqli_query($con, "INSERT INTO jabatan values('','".$customer_id."','".$department."','".$unit."','".$block."','".$level."', ".$depmq."')");
    ...
    $qry = mysqli_query($con, "UPDATE jabatan SET customer_id='".$customer_id."', jabatan='".$department."', unit='".$unit."', blok='".$block."', tingkat='".$level."', peruntukan_bulanan='".$depmq."' where jabatan_id='".$department_id."'");
          ...
          <div class="col-lg-9 col-md-9 col-sm-9 col-xs-12">
                <div class="panel panel-info">
                   <div class="panel-heading">
                       <div class="panel-title">Add PPUKM Departments</div>
                    </div>
                   <div class="panel-body">
                       <?php echo $msg; ?>
                   <form id="form_department" class="form-horizontal" action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post">
                       ...
                       <div style="margin-bottom: 25px" class="input-group">
                           ...
                           <input type="text" class="form-control" name="depmq" id="depmq" value="<?php echo $depmq; ?>"/>
                       </div>
            ...
            $('#year').on('change',function(){
              ...
              $('input[class="form-control"]').focus(function() {
                ...
                $("#btn_save, #btn_edit").click(function(e){
                    if($("#customer_id").val() == ''){
                        $("#customer_id").css("border-color","#DA1908");
                        $("#customer_id").css("background","#F2DEDE");
                        e.preventDefault();
                    }
                    if($("#department").val() == ''){
                        $("#department").css("border-color","#DA1908");
                        $("#department").css("background","#F2DEDE");
                        e.preventDefault();
                    }
                    if($("#depmq").val() == ''){
                        $("#depmq").css("border-color","#DA1908");
                        $("#depmq").css("background","#F2DEDE");
                        e.preventDefault();
                    }else{
                        $('form_department').unbind('submit').submit();
                    }
                });
            });
    </script>
