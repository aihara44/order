<?php
    ...
    $qry = mysqli_query($con, "DELETE FROM users WHERE user_id ='" .mres($con, $_GET["delete_id"])."'");
                    ...
                    <tbody>
                       <?php
                        ...
                        if (isset($_POST["btn_search"])){
                            $qry = mysqli_query($con, "SELECT * FROM users where username like '%".mres($con, $_POST["search_text"])."%' order by user_id asc");
                        }
                        $qry = mysqli_query($con, "SELECT * FROM users ORDER BY user_id asc");
                        while($row=mysqli_fetch_array($qry,MYSQLI_ASSOC)){
                            echo '<tr>';
                            echo '<td>'.$row["nama_staf"]."</td><td>".$row["username"]."</td><td>".$row["jawatan"]."</td><td><a href='addusers.php?edit_id=".$row["user_id"]."' class='btn btn-primary'>Edit</a> | <a href='?delete_id=".$row["user_id"]."' class='btn btn-danger' onclick=\"return confirm('Are you sure you want to delete this item?');\">Delete</a></td>";
                            echo '</tr>';
                        }
                        ?>
